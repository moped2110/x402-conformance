...
### Fixed
- **FA-ERR-001 was failing conformant facilitators.** `KNOWN_ERROR_CODES` was
  vendored from the `ErrorReasons` Zod enum, which was once the only vocabulary a
  wire field could hold. Upstream's package split froze that enum under `legacy/`
  and moved new codes into the per-mechanism packages — **344 codes across 28
  files** that we were rejecting from implementations doing nothing wrong. The
  sharpest case is a spelling: the current EVM package returns
  `invalid_exact_evm_authorization_value`, while the enum only ever had
  `invalid_exact_evm_payload_authorization_value`.

  The drift guard did not catch this because it watched `x402Specs.ts`, and that
  file never changed — it still has exactly 41 codes. A guard pointed at a source
  upstream has abandoned reports success, not safety. Codes are now generated by
  `tools/sync_error_registry.py` into a committed `error_registry.py`, and the
  drift test re-extracts from a live clone and diffs it, naming the offending
  codes.

### Added
- **RS-SEC-012 — paywall bypass by re-encoding the request path (CRITICAL, passive).**
  Upstream fixed this class three times in three weeks: x402#3036 (TypeScript),
  #3055 (Python), #3044 (Go). A path the paywall matcher and the router disagree
  about misses the gate and still reaches the handler, so the resource is served
  with no verification and no settlement. Probes line terminators in the wildcard
  tail, percent-encoded separators, dot-segments and percent-encoded unreserved
  characters, with a control probe so a catch-all endpoint SKIPs instead of
  producing a false critical. Both terminator placements are probed: Python's `$`
  forgives a *trailing* newline, so only an embedded one exposes a missing
  `re.DOTALL`, while JavaScript needs the trailing form. Verified against a live
  server carrying upstream's actual pre-fix regex, which leaks its secret on
  `/data%0Aa`.
- **DI-004 — external `$ref`/`$id` in a catalogued Bazaar schema (MAJOR).** Now
  normative (x402#3039): references must be same-document fragments, and
  facilitators must not resolve external ones, because a validator dereferences
  them while compiling a client-supplied schema (CWE-918). The check reports and
  never dereferences.
- **RS-PR-023/024 — builder-code (MINOR).** x402#3027/#2994 turned a format-only
  extension into one with per-party service-code reservations. The
  server-declared half is checkable from the challenge: app-code format, and the
  server staying inside `MAX_SERVER_SERVICE_CODES`.
- **RS-HS-008 — the paid 200 is not shared-cacheable (MINOR).** RS-HS-007 covers
  the 402; this covers the response carrying content the client just paid for,
  which a shared cache would serve to someone who did not (x402#2990).
- Celo Sepolia (`eip155:11142220`) in the signing allowlist. Celo and Flare
  *mainnet* are deliberately excluded — the safety policy rejects mainnets before
  payload construction, with no override.

### Changed
- Catalog and README counts 69 → 74.
- `--resource-marker` now applies passively too, narrowing RS-SEC-012 to a body
  that really contains the protected content. Its "no effect without `--active`"
  notice is gone because that is no longer true.
- Support matrix: Starknet `exact` recorded as **planned** (x402#2849);
  builder-code moved from passive-only to partially supported; the SVM row
  records that `extra.recentBlockhash` is a non-binding hint (x402#2937) and that
  the FA-SVM tamper set was checked for a blockhash case and has none.
- Upstream reviewed pin `61349de` → `c7e0ac8`.

### Verified, no change needed
- FA-SET already proves the exact on-chain `Transfer` and SKIPs without an RPC,
  so the upstream move away from trusting `receipt.status` (x402#2385/#2727/#3032)
  found us already ahead.
