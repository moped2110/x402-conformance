...
**Planned (in this catalog, not yet shipped):** RS-NEG-010, RS-SEC-008, FA-VER-001. RS-SEC-003 now ships as a MINOR *advisory* cross-resource-binding probe (single-request resource relabel; overlaps the RS-NEG-013 "validate `accepted` against your own offer" principle applied to `resource`) — it never gates the verdict. The full economic exploit (replay of a *settled* payment across resources) needs two resources + settlement and is out of the single-endpoint black-box scope.
