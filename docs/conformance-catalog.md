...
| RS-PR-013 | payTo/asset match the network's CAIP-2 namespace | No `eip155`/`solana`/`xrpl` cross-wiring (an EVM `0x` address on a Solana or XRPL rail, or a non-EVM address on an eip155 rail) | CORE §11.1 + N1/N2 | M | implemented |
