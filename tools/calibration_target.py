...
Then:
    ...
    # correct server -> every RS-NEG / RS-SEC-010 check PASS
...
def payment_required(port: int) -> dict:
    return {
        "x402Version": 2,
...
def make_handler(bugs: set[str], port: int) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if sig is None:
                self._send(402, {"PAYMENT-REQUIRED": _b64(payment_required(port))}, b"{}")
                return
            try:
...
if __name__ == "__main__":
    ...
    for a in sys.argv[1:]:
        if a.startswith("--bug-no-"):
            bugs.add(a.removeprefix("--bug-no-"))
        elif a.isdigit():
            port = int(a)
